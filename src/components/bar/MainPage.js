...
class HomePage extends React.Component {
  constructor(props, context) {
    ...
    this.state = {
      ...
      barHeight: 22,
      backColor: '#eeeeee',
      barColor: ['#16b7d4', 'orange', 'yellow', 'silver', 'aqua'],
  ...
  setBars() {
    ...
    let barList = bars.map(function (bar, index) {
      if (bar != null) {
        return (
          <Bar key={'bar' + index} height={barHeight} limit={limit} value={bar} backColor={backColor} barColor={barColor[Math.floor(Math.random() * barColor.length)]} completeColor={completeColor} foreColor={foreColor} />
        );
      }
    });
  ...
  setControls() {
    ...
    let buttonList = buttons.map(function (button, index) {
      if (button != null) {
        ...
        return (
          <button key={'button' + index} type="button" onClick={this.onControlClick.bind(this, val)} style={{color: "#ffffff",backgroundColor: "#00695c"}}>{label}</button>
        );
      }
    }.bind(this));
      ...
      <div>
        <section id="content">
          <div className="container">
            <h3 className="text-center">Front End Assignement : Progress Bars in React JS</h3>
            <hr />
